    ...
    try {
      ...
      for (const date in grouped) {
        ...
        for (const doc of rows) {
          ...
          grandTotalCpp1 += doc.total_reclaiming || 0;
          grandTotalCpp3 += doc.cpp3total_reclaiming || 0;
      ...
      let totalCol = 3 + cpp1Names.length;
      sheet.getCell(g, totalCol).value = grandTotalCpp1;

      sheet.getCell(g, totalCol + 1).value = grandY4;
      sheet.getCell(g, totalCol + 2).value = grandY4A;
      sheet.getCell(g, totalCol + 3).value = grandY127;

      let cpp3Start = headers.indexOf("") + 2;

      cpp3Names.forEach((c, i) => {
        sheet.getCell(g, cpp3Start + i).value = grandCpp3[c];
      });

      let cpp3TotalCol = headers.indexOf("CPP3 TOTAL") + 1;
      sheet.getCell(g, cpp3TotalCol).value = grandTotalCpp3;

      let pathACol = headers.indexOf("PATH-A") + 1;
      let pathBCol = headers.indexOf("PATH-B") + 1;

      sheet.getCell(g, pathACol).value = grandPathA;
      sheet.getCell(g, pathBCol).value = grandPathB;
